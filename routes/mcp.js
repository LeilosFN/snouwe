...
app.post("/fortnite/api/game/v2/profile/*/client/SetReceiveGiftsEnabled", verifyToken, async (req, res) => {
    ...
    if (!await profileManager.validateProfile(req.query.profileId, profiles)) return error.createError(
        "errors.com.leilos.modules.profiles.operation_forbidden",
    ...
    if (req.query.profileId != "common_core") return error.createError(
        "errors.com.leilos.modules.profiles.invalid_command",
...
app.post("/fortnite/api/game/v2/profile/*/client/GiftCatalogEntry", verifyToken, async (req, res) => {
    ...
    if (missingFields.fields.length > 0) return error.createError(
        "errors.com.leilos.validation.validation_failed",
    ...
    if (req.body.personalMessage.length > 100) return error.createError(
        "errors.com.leilos.string.length_check",
    ...
    if (!validGiftBoxes.includes(req.body.giftWrapTemplateId)) return error.createError(
        "errors.com.leilos.giftbox.invalid",
    ...
    if (req.body.receiverAccountIds.length < 1 || req.body.receiverAccountIds.length > 5) return error.createError(
        "errors.com.leilos.item.quantity.range_check",
    ...
    if (checkIfDuplicateExists(req.body.receiverAccountIds)) return error.createError(
        "errors.com.leilos.array.duplicate_found",
    ...
    for (let receiverId of req.body.receiverAccountIds) {
        if (typeof receiverId != "string") return error.createError(
            "errors.com.leilos.array.invalid_string",
        ...
        if (!sender.list.accepted.find(i => i.accountId == receiverId) && receiverId != req.user.accountId) return error.createError(
            "errors.com.leilos.friends.no_relationship",
    ...
    if (!findOfferId) return error.createError(
        "errors.com.leilos.fortnite.id_invalid",
        ...
        case /^BR(Daily|Weekly)Storefront$/.test(findOfferId.name):
            if (findOfferId.offerId.prices[0].currencyType.toLowerCase() == "mtxcurrency") {
                ...
                for (let key in profile.items) {
                    ...
                    if (profile.items[key].quantity < price) return error.createError(
                        "errors.com.leilos.currency.mtx.insufficient",
            ...
            for (let receiverId of req.body.receiverAccountIds) {
                ...
                if (!common_core.stats.attributes.allowed_to_receive_gifts) return error.createError(
                    "errors.com.leilos.user.gift_disabled",
                ...
                for (let itemGrant of findOfferId.offerId.itemGrants) {
                    for (let itemId in athena.items) {
                        if (itemGrant.templateId.toLowerCase() == athena.items[itemId].templateId.toLowerCase()) return error.createError(
                            "errors.com.leilos.modules.gamesubcatalog.purchase_not_allowed",
            ...
            for (let value of findOfferId.offerId.itemGrants) {
                ...
                for (let itemId in athena.items) {
                    if (value.templateId.toLowerCase() == athena.items[itemId].templateId.toLowerCase()) return error.createError(
                        "errors.com.leilos.offer.already_owned",
...
app.post("/fortnite/api/game/v2/profile/*/client/MarkItemSeen", verifyToken, async (req, res) => {
    ...
    if (req.query.profileId == "athena") profile.stats.attributes.season_num = 0;
...
app.post("/fortnite/api/game/v2/profile/*/client/SetItemArchivedStatusBatch", verifyToken, async (req, res) => {
    const profiles = await Profile.findOne({ accountId: req.user.accountId });
    let profile = profiles.profiles[req.query.profileId];
    const memory = functions.GetVersionInfo(req);
        ...
        if (process.env.B_ENABLE_BATTLEPASS === "false") {
             for (const key in profile.items) {
                 ...
                 if (templateId.startsWith("BattlePass:") ||
                     templateId.startsWith("ChallengeBundle:") ||
                     templateId.startsWith("Quest:") ||
                     templateId.startsWith("AthenaSeason:")) {
...
app.post("/fortnite/api/game/v2/profile/*/client/SetBattleRoyaleBanner", verifyToken, async (req, res) => {
    ...
    if (!HomebaseBannerIconID) return error.createError(
        "errors.com.leilos.fortnite.item_not_found",
...
app.post("/fortnite/api/game/v2/profile/*/client/PutModularCosmeticLoadout", verifyToken, async (req, res) => {
    const profiles = await Profile.findOne({ accountId: req.user.accountId });
    let profile = profiles.profiles[req.query.profileId];
...
app.post("/fortnite/api/game/v2/profile/*/client/:operation", verifyToken, async (req, res) => {
    ...
    if (req.query.profileId == "common_core") {
        for (let key in profile.items) {
             if (profile.items[key].templateId == "Currency:MtxPurchased") {
    ...
    if (req.query.profileId == "athena") {
        if (process.env.B_ENABLE_BATTLEPASS === "false") {
             ...
             if (profile.stats.attributes) {
                 ...
                 if (profile.stats.attributes.book_purchased !== false) {
                     profile.stats.attributes.book_purchased = false;
                     statsChanged = true;
                 }
                 if (profile.stats.attributes.book_level !== 1) {
                     profile.stats.attributes.book_level = 1;
                     statsChanged = true;
                 }
                 
                 // Force stat updates if changed (simplified, sending all stats might be heavy but safer for sync)
                 if (statsChanged) {
                      ApplyProfileChanges.push({
                        "changeType": "statModified",
                        "name": "book_purchased",
        ...
        if (!profile.stats.attributes.ranking_info) {
            profile.stats.attributes.ranking_info = {};
            rankChanged = true;
        }
        if (profile.stats.attributes.ranking_info.rank !== 1) {
            profile.stats.attributes.ranking_info.rank = 1;
            profile.stats.attributes.ranking_info.division = 1;
            profile.stats.attributes.ranking_info.points = 0;
            rankChanged = true;
        }
        ...
        if (rankChanged) {
             ApplyProfileChanges.push({
                "changeType": "statModified",
                "name": "ranking_info",
                "value": profile.stats.attributes.ranking_info
             });
        }
    }
    ...
    switch (req.params.operation) {
        ...
        case "SetLoadoutShuffleEnabled":
            if (typeof req.body.shuffleEnabled == "boolean") {
                profile.stats.attributes.loadout_shuffle_enabled = req.body.shuffleEnabled;

                ApplyProfileChanges.push({
                    "changeType": "statModified",
                    "name": "loadout_shuffle_enabled",
                    "value": profile.stats.attributes.loadout_shuffle_enabled
                });
            }
        break;

        case "PutModularCosmeticLoadout": break;

        default:
            error.createError(
                "errors.com.leilos.fortnite.operation_not_found",
